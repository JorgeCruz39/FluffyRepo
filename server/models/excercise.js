const data = require('../data/excercise.json');
const { connect, ObjectId } = require('./mongo');

const COLLECTION_NAME = 'excercise';
...
async function search(searchTerm, page = 1, pageSize = 30) {
    ...
    const query = {
        $or: [
            { exercise: { $regex: searchTerm, $options: 'i' } },
...
async function seed() {
    const col = await collection();
    const result = await col.insertMany(data.excercise);
